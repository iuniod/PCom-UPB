...
void init_udp_connection(Server *server) {
  ...
  DIE((server->udp_sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0,
  ...
  int enable = 1;
  DIE(setsockopt(server->udp_sockfd, SOL_SOCKET, SO_REUSEADDR, &enable,
                 sizeof(int)) < 0,
  ...
  DIE(bind(server->udp_sockfd, (const struct sockaddr *)&server->udp_socket,
...
void init_tcp_connection(Server *server) {
  /* Creating socket file descriptor */
  DIE((server->tcp_sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0,
      "socket creation failed");

  /* Make ports reusable, in case we run this really fast two times in a row */
  int enable = 1;
  DIE(setsockopt(server->tcp_sockfd, SOL_SOCKET, SO_REUSEADDR, &enable,
                 sizeof(int)) < 0,
      "setsockopt(SO_REUSEADDR) failed");

  /* Disable Nagle's algorithm */
  int flag = 1;
  DIE(setsockopt(server->tcp_sockfd, IPPROTO_TCP, TCP_NODELAY, &flag,
                 sizeof(int)) < 0,
  ...
  /* Bind the socket with the server address */
  DIE(bind(server->tcp_sockfd, (const struct sockaddr *)&server_addr,
